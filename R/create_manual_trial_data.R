...
create_manual_trial_data2 <- function(data,
  ...
  dvdata <- data2  %>%
    ...
    mutate(.lag = time - lag(time),
           .lag = ifelse(is.na(.lag), samplerate, .lag)) %>% #approximate lag with sample rate if needs be
    group_by(ID, Trial) %>%
    mutate(Left = ifelse(Direction == 'left', .lag, 0), #no direction switch
           Right = ifelse(Direction == 'right', .lag, 0),
  ...
  dvdata2 <- dvdata %>%
    ...
    group_by(ID, Trial) %>%
    mutate(CURRENT_FIX_INTEREST_AREA_LABEL = case_when(ChangeSide == Direction ~ 'target',
                                                       ChangeSide != Direction ~ 'distractor',
                                                       Direction == 'away' | Direction == 'none' | Direction == 'noface' ~ 'trackloss',
                                                       TRUE ~ 'trackloss')) %>%
    ...
    mutate(video_condition = word(Trial, 2, sep = '_'),
           video_condition = ifelse(is.na(video_condition), 'video', video_condition))
...
#' create a trial report from a robocode pingu
#'
#' This was designed to work with icatcher frame-by-frame reports and the VWM trial.
#' I can't guarantee it will bring out what you want beyond that so please check
#' the output carefully.
...
create_pingu_data <- function(data,
                                      write = F){
  ...
  message('no natural target, treating left as target \n')
  ...
  dvdata <- data2  %>%
    ...
    # mutate(Look_Ord = TrialLook.looking_ordinal) %>%
    select(-.lag) %>%
    mutate(Target = ifelse(Direction == 'left', 1, 0),
           Distractor = ifelse(Direction == 'right', 1, 0),
           Trackloss = ifelse(Direction == 'away' | Direction == 'none' | Direction == 'noface', 1, 0),
           target = ifelse(is.na(Target), 0, Target),
           distractor = ifelse(is.na(Distractor), 0, Distractor),
           trackloss = ifelse(is.na(Trackloss), 0, Trackloss))
  ...
  dvdata2 <- dvdata %>%
    ...
    summarise(Left = sum(Left, na.rm = T),
              ...
              Dis = sum(distractor),
              TLT = Left + Right,
              max = max(timestamp_new, na.rm = T)) %>% #not sum
    mutate(PercLook = TLT/max,
           MLD = TLT/Looks,
           SR = Switch/(TLT/1000),
           CP = Left/TLT,
           Both = ifelse(Left > 0 & Right > 0, 'Y', 'N')) %>%
